...
export function SectionHeading({ eyebrow, title, subtitle, center }: { eyebrow?: string; title: string; subtitle?: string; center?: boolean }) {
  return (
    <div className={cn("max-w-2xl", center && "mx-auto text-center")}>
      ...
      <h2 className="mt-3 text-3xl font-semibold tracking-tight text-foreground sm:text-4xl text-balance">{title}</h2>
  ...
  return (
    <div className="relative overflow-hidden border-b border-border">
      ...
      <Section className="relative py-20 sm:py-28">
        ...
        <h1 className="mt-4 max-w-3xl text-4xl font-semibold tracking-tight text-foreground sm:text-5xl text-balance">{title}</h1>
    ...
    <Section className="py-20">
      <div className="relative overflow-hidden rounded-3xl bg-foreground px-8 py-16 text-center text-background sm:px-16">
        ...
        <div className="relative">
          <Badge className="bg-background/15 text-background">Book a Demo</Badge>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3">
            <Link href="/book-demo" className="rounded-lg bg-background px-6 py-3 text-sm font-semibold text-foreground transition-transform hover:scale-[1.02]">Book a Demo</Link>
            <Link href="/app" className="rounded-lg border border-background/30 px-6 py-3 text-sm font-semibold text-background transition-colors hover:bg-background/10">Explore the Platform</Link>
          </div>
        </div>
      </div>
    </Section>
